...
const {
  ...
  PROVIDER_FIELDS,
  BANK_ACCOUNT_FIELDS,
} = require("./fields/entityFields.js");
...
const partnerService = {
    ...
    try {
      ...
      if (data.bankAccounts?.length > 0) {
        for (const account of data.bankAccounts) {
          ...
          const banks = await bankService.searchBanksByNameIlike(
          ...
          if (banks.statusCode === 404) {
            ...
            const bankId = await bankService.createBank(
              {
          ...
          if (!bankAccounts.includes(bankAccountData.acc_number)) {
            ...
            const bankAccountId = await bankAccountService.createBankAccount(
              bankAccountData,
              user
            );
            results.push({
            ...
            bankAccountInvalid.push(
              `La cuenta bancaria ${bankAccountData.acc_number} está duplicada en la solicitud y no fue creada.`
            );
          }
        }
      }
      // 3. Retornar resultado
      const partner = await this.getOneClient(
        clientId.data,
        undefined,
        undefined,
        user
      ...
      return {
        statusCode: 200,
        message: "Cliente y cuentas bancarias creadas exitosamente",
        data: {
  ...
  async updatePartner(id, newData, user) {
    try {
      ...
      if (newData.company_id) {
        const company = await companyService.companyExists(
          newData.company_id,
          user
        );
  ...
  async deleteClient(id, company_id, user) {
    // Verificamos la sesión
    try {
      const ids = await this.getOneClient(
        Number(id),
        Number(company_id),
        undefined,
        user
  ...
  async updateClientWithCompanyValidation(
    ...
    try {
      ...
      const updateClients = await this.updateClients(
        id,
        novoCliente,
        companyIdSearch,
        user
      ...
      return {
        statusCode: 500,
        message: "Error al validar la compañía",
        data: [],
      };
    }
  },
  ...
  async getOneProvider(id, company_id, user) {
    try {
      ...
      return {
        ...
        message: "Cliente encontrado",
        data: clients.data[0],
      }; // Asumiendo que el ID es único, se retorna el primer cliente
    } catch (error) {
      // Propagar el error si es necesario
  ...
  async getPartners(company_id, type, user) {
    try {
      ...
      const partners = await connector.executeOdooQuery(
        "object",
        "execute_kw",
        [
          user.db,
